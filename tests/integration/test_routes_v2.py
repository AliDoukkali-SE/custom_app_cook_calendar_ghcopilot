...
def json_repository(tmp_path):
    data_file = tmp_path / "meals-v2.json"
    return JsonMealRepository(file_path=str(data_file))
...
async def client(test_app):
    ...
    async with AsyncClient(
        transport=transport,
        base_url="http://test",
        headers={"X-User-Id": TEST_OWNER_ID_STR},
    ) as async_client:
        yield async_client
...
class TestMealsRoutesV2:
    async def test_create_and_list_meal(self, client: AsyncClient):
        ...
        assert created["id"]
        assert created["owner_id"] == TEST_OWNER_ID_STR
    ...
    async def test_create_rejects_same_date_slot_for_same_owner(self, client: AsyncClient):
        ...
        first_response = await client.post("/meals/", json=payload)
        assert first_response.status_code == 201

        second_response = await client.post("/meals/", json=payload)
        assert second_response.status_code == 409
        assert "owner/date/slot" in second_response.json()["detail"]
